...
public:
    ...
    GraafMetTakdata(const GraafMetTakdata<RT,Takdata>& gr):GraafMetTakdata(gr.aantalKnopen()){
    	std::cout<<"ik voer een copy constructor uit"<<std::endl;
		for(int i=0; i<gr.aantalKnopen(); i++){
			for(std::map<int,int>::const_iterator it= gr.knopen[i].begin(); it!=gr.knopen[i].end(); ++it ){
				int naar = it->first;
				int van = i;
				this->voegVerbindingToe(van,naar,*gr.geefTakdata(van,naar));
			}
		}    	
	}
	...
	Extra: Dijkstra
	*/
	vector<int> Dijkstra(int startknoop)const;

private:
	void onderzoek_buren(int knoopnr,	priority_queue<pair<double,int>,vector<pair<double,int>>,std::greater<pair<double,int>>>&pq, vector<int>& mindist
		,vector<Kleur>& kleuren, vector<int>& ouders)const;
...
/*
Extra implemenatatie voor Johnson: Copy constructor + Dijkstra uitvoeren
*/

template<RichtType RT,class Takdata>
vector<int>  GraafMetTakdata<RT,Takdata>::Dijkstra(int startknoop)const{
	vector<Kleur> kleuren(this->aantalKnopen());
	vector<int> ouders(this->aantalKnopen());
	vector<int>mindist(this->aantalKnopen());
	...
	onderzoek_buren(startknoop,pq,mindist,kleuren,ouders);
	
	while(!pq.empty()){
		pair<double,int> p = pq.top();
		pq.pop();
		if(kleuren[p.second]!=ZWART){
				// voegVerbindingToe(ouders[p.second],p.second,*geef_takdata(ouders[p.second],p.second));	
//				 cout<<"gh: voeg verbinding toe van: "<<ouders[p.second]<<" naar "<<p.second<<"\n";
				onderzoek_buren(p.second,pq,mindist,kleuren,ouders);
		}
...
template<RichtType RT,class Takdata>
void GraafMetTakdata<RT,Takdata>::onderzoek_buren(int knoopnr,priority_queue<pair<double,int>,vector<pair<double,int>>,greater<pair<double,int>>>&pq, vector<int>& mindist
			,vector<Kleur>& kleuren, vector<int>& ouders)const{
	kleuren[knoopnr] = ZWART;
	for(std::map<int,int>::const_iterator it = this->knopen[knoopnr].begin();it!=this->knopen[knoopnr].end();it++){
...
//knoop is al grijs, maar kan nu een lichtere verbinding krijgen
			int nieuweafstand=mindist[knoopnr]+*geefTakdata(knoopnr,it->first);
