...
class Node{
	public: 
		Node(const T& data_, Node<T>* left, Node<T>* right):data(data_),left_child(left),right_brother(right){
		};
		Node<T>* left_child;
...
template<class T>
void Node<T>::schrijf_inorder(ostream& os)const{
      if (left_child != nullptr){
      	left_child->schrijf_inorder(os);
	  }
      if (right_brother !=nullptr) {
          right_brother->schrijf_inorder(os);
	  }
		os<<data<<"; ";
    }
...
public:
   ...
   * the heap. immutable = niet wijzigbaar
   */
  const T& top() const;
...
struct BinomialNode {
	...
	void schrijf_inorder(ostream& os)const{
      if (mChild != nullptr){
      	mChild->schrijf_inorder(os);
	  }
      if (mRight !=nullptr) {
          mRight->schrijf_inorder(os);
	  }
		os<<mValue<<"; ";
    }
  ...
  void BinomialHeapMerge(std::vector<BinomialNode<T>*>& lhs, 
    ...
    // zorgt ervoor dat geen nieuwe vector aangemaakt wordt en vermijd op deze manier problemen.
    rhs.clear();
    lhs = result;
  }
 ...
 * Elk laagste element bevind zich in de root van elke boom
 */
template <typename T>
...
void BinomialHeap<T>::pop() {
  ...
  typename std::vector<detail::BinomialNode<T>*>::iterator minElem = std::min_element(mTrees.begin(), mTrees.end(), 
...
template <typename T>
void BinomialHeap<T>::schrijf()const{
	for(int i=0; i<mTrees.size(); i++){
		cout<<"i-de boom:\t";
		mTrees[i]->schrijf_inorder(cout);
	}
