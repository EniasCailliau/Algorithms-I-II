...
class Group{
	public:
		Group(int nummer, int aantal, int groupid) : amount(aantal), groupNameID(nummer),groupID(groupid){
		}
		int getGroupNameID(){
			return groupNameID;
		}
...
class SinterklaasBoot : public SimulatedAnnealing<vector<int> >{
	...
	private:
		...
		vector<int > pickAtRandom(const vector<int> &  old){
	    ...
	    double f(const vector<int>& vect){
	    	double counter = 0;
	    ...
	    double p(double T, double deltaf){
			return exp(-deltaf/T); 
