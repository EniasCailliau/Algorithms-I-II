...
protected:
/////////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	void behandel_knoop(int);
  	virtual void prebehandel_knoop(int i) ;
    virtual void postbehandel_knoop(int i) ;
/////////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
template <GraafType TYPE>
void Graaf<TYPE>::behandel_knoop (int i){
	ontdekt[i] = true;
	prebehandel_knoop(i);
	for(std::map<int,int>::iterator it= knopen[i].begin();it!=knopen[i].end();it++){
		if(!ontdekt[it->first]){
			behandel_knoop(it->first);
		}
	}
	postbehandel_knoop(i);
	postnummering[i]=postnummer++;
	
}

template <GraafType TYPE>
void Graaf<TYPE>::diepte_eerst_zoeken(){
	postnummer=1;
	postnummering.resize(aantal_knopen());
	ontdekt.resize(knopen.size());
...
template <GraafType TYPE>
void Graaf<TYPE>::prebehandel_knoop(int i){
	
}
template <GraafType TYPE>
void Graaf<TYPE>::postbehandel_knoop(int i){
