...
public:
    ...
    vector<int>& bepaal_postnummering(){
    	overloop_diepte_eerst();
    	// cout<<postnummer.size();
    	return postnummer;
	}
	
	void behandel_knoop(int i, int compnr){
		componentnr[i]=compnr;
		ontdekt[i]=true;
		for (Knoop::const_iterator it=knopen[i].begin(); it!=knopen[i].end(); ++it){
            int naar = it->first;
            	if(!ontdekt[naar]){
					behandel_knoop(naar, compnr);
			}
                
        }
		postnummer.push_back(i);
	}
...
protected:
    ...
    std::vector<int> postnummer;
    std::vector<int> prenummer;
static int postnummering;
};
...
: knopen(n), nverbindingen(0), postnummer(n)
{ init(n); }
...
void Graaf<TYPE>::init(int n)
{
    ...
    ontdekt.resize(n);
   	componentnr.clear();
   	componentnr.resize(n);
   	postnummer.clear();
   	postnummer.reserve(n);
    nverbindingen = 0;
}
